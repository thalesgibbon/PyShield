...
with open(r'C:\Users\usuario_itau\Desktop\CRIADAS\Distritos_De_Policia.pickle', 'rb') as handle:
    distpol = pickle.load(handle)

distpol = distpol[['Latitude', 'Longitude']]
distpol.columns = ['LATITUDE', 'LONGITUDE']

base_all.LATITUDE = base_all.LATITUDE.apply(lambda x: float(str(x).replace(',','.')))
distpol.LATITUDE = distpol.LATITUDE.apply(lambda x: float(str(x).replace(',','.')))

base_all.LONGITUDE = base_all.LONGITUDE.apply(lambda x: float(str(x).replace(',','.')))
distpol.LONGITUDE = distpol.LONGITUDE.apply(lambda x: float(str(x).replace(',','.')))
...
for i in base_all.values.tolist():
    l = 1
    for j in distpol.values.tolist():
        dist = DistanciaKm(i[2], j[0], i[3], j[1], str(k)+'-'+str(l))
        if dist < 1:
...
with open(r'C:\Users\usuario_itau\Desktop\PRE_BASES\cross_distpol.pickle', 'wb') as handle:
